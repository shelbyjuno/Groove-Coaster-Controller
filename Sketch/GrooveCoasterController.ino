...
void setupPins()
{
  ...
  joystickUP_2.attach(PIN_UP_2, INPUT_PULLUP);
  ...
  buttonA_2.attach(PIN_A_2, INPUT_PULLUP);
  ...
  joystickUP_2.interval(MILLIDEBOUNCE);
  ...
  buttonA_2.interval(MILLIDEBOUNCE);
...
void buttonRead()
{
  ...
  if (joystickUP.changed() || joystickUP_2.changed() || joystickDOWN.changed() || joystickLEFT.changed() || joystickRIGHT.changed())
  {

    internalButtonStatus[BUTTONUP] = !joystickUP.read() || !joystickUP_2.read();
  ...
  if (buttonA.update() || buttonA_2.update())
  {
    buttonStatus[BUTTONA] = buttonA.fell() || buttonA_2.fell();
  }
